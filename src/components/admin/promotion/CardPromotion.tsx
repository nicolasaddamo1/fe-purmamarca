...
export function CardPromotion({
  ...
  category_ids?.forEach((cat_id) => {
    categories.map(
      (categorie) =>
        categorie.id === cat_id && categories_names.push(categorie.name)
    );
  });
      ...
      <Card.Meta
        ...
        description={
          <div className="min-h-24 text-sm">
            <p>Inicio: {dayjs(start_date).format("DD/MM/YYYY")}</p>
            <p>Fin: {dayjs(expiration_date).format("DD/MM/YYYY")}</p>

            {categories_names.length >= 1 && (
              <p>Categorías: {categories_names.join(", ")}</p>
            )}
            {promo_percentage && <p>Descuento: {promo_percentage}%</p>}
          </div>
        }
      />
    </Card>
