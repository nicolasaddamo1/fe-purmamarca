...
function Product({
  ...
  return (
    <div
      className={`group flex h-80 md:h-auto flex-col justify-between hover:shadow-2xl rounded-xs  ${
        available
          ? "outline-[#76644c67] hover:outline-[#76644c]"
          : "outline-neutral-400 hover:outline-neutral-700"
      } outline-2  w-64 md:w-56  duration-200`}
    >
      <div className={` ${available ? "bg-[#dbc7ab]" : "bg-gray-200"}`}>
        <img
          className="m-auto w-full md:w-32 md:h-36"
          src={imageUrl}
          title={name}
          alt={name}
        />
      ...
      <div className="flex flex-col items-center md:items-start gap-0.5 md:gap-2 px-3 py-2 w-full md:w-52 font-medium">
        ...
        >
          <h5
            className={` text-2xl w-full font-sans  overflow-ellipsis truncate hover:text-blue-700 duration-200 md:text-xl text-gray-800 ${
        ...
        <div
          className={`text-2xl md:text-base ${
            available ? "text-green-800" : "text-gray-800"
          }`}
        >
          {priceOnSale ? (
